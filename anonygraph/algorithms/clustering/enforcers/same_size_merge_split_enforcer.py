...
class SameSizeMergeSplitEnforcer(BaseEnforcer):
    def __init__(self, args):
        super().__init__(KMEANS_PARTITION_ENFORCER, args)

        self.max_dist = float(args[0])

    def calculate_k(self, id2k_dict):
        k_vals = list(id2k_dict.values())
        k = max(k_vals)

        return k

    def call(self, clusters, dist_matrix, entity_id2idx_dict, entity_idx2id_dict, entity_id2k_dict):
        k = self.calculate_k(entity_id2k_dict)

        # invalid removal
        logger.info("before enforcing")
        tutils.print_invalid_and_big_clusters(clusters, entity_id2k_dict)

        valid_clusters, removed_entity_ids = get_valid_clusters(clusters, k)
        ...
        # raise Exception()
        # merge
        merge_clusters(valid_clusters, removed_entity_ids, self.max_dist, dist_matrix, entity_id2idx_dict, k)
        ...
        # # split clusters
        split_big_clusters(valid_clusters, dist_matrix, entity_id2idx_dict, entity_idx2id_dict, k)
...
def get_valid_clusters(clusters, k):
...
def merge_clusters(clusters, entity_ids, max_dist, dist_matrix, entity_id2idx_dict, k):
    real_max_dist = calculate_real_max_dist(max_dist, dist_matrix)

    # for entity_id in entity_ids:
    for entity_id in entity_ids:
        closest_cluster = find_closest_cluster(entity_id, clusters, real_max_dist, dist_matrix,  entity_id2idx_dict, k)
...
def find_closest_cluster(entity_id, clusters, max_dist, dist_matrix, id2idx_dict, k):
...
def split_big_clusters(clusters, dist_matrix, entity_id2idx_dict, entity_idx2id_dict, k):
    index = 0
    ...
    while(index < len(clusters)):
        ...
        if len(current_cluster) >= k * 2:
            new_clusters_list = split_big_cluster(current_cluster, k, dist_matrix, entity_id2idx_dict, entity_idx2id_dict)
